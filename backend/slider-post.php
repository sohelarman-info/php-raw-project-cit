 ...
 if ($_SERVER['REQUEST_METHOD'] == 'POST') {
   ...
   if (in_array($ext, $allow_format)) {
     if ($upload_images['size'] <= 999999) {
       $insert            = "INSERT INTO slider(title, content) VALUES ('$title','$content')";
       ...
       $update            = "UPDATE slider SET image = '$image_name' WHERE id = $last_id";
       $query             = mysqli_query($db, $update);

      if ($insert_query) {
        $_SESSION['insert'] = 'Add your slider successfully.';
